...
function dueMeta(date: string): { text: string; cls: string; dot: string } {
  ...
  if (diff < 0) return { text: `逾期 ${-diff} 天`, cls: 'text-red-600 bg-red-50', dot: 'bg-red-500' }
  if (diff === 0) return { text: '今天到期', cls: 'text-amber-600 bg-amber-50', dot: 'bg-amber-500' }
  if (diff <= 3) return { text: `${diff} 天後`, cls: 'text-amber-600 bg-amber-50', dot: 'bg-amber-500' }
  return { text: `${diff} 天後`, cls: 'text-blue-600 bg-blue-50', dot: 'bg-blue-500' }
}
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h2 className="font-semibold text-gray-700 text-sm flex items-center gap-1.5">
          <CalendarClock className="w-4 h-4" /> 保養待辦
          {overdueCount > 0 && (
            <span className="text-xs px-1.5 py-0.5 rounded bg-red-100 text-red-700">逾期 {overdueCount}</span>
          )}
        </h2>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-center py-8 text-sm text-gray-400">
          {tasks.length === 0 ? '近期沒有待辦保養' : '找不到符合的機器'}
        </p>
      ) : (
        <div className="space-y-2">
          {filtered.map(t => {
            const meta = dueMeta(t.scheduled_date)
            return (
              <div
                key={t.record_id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-800 truncate">
                    {t.machine_code ? `[${t.machine_code}] ` : ''}{t.machine_name}
                  </p>
                  <p className="text-xs text-gray-500">
                    {t.scheduled_date}
                    {t.pm_type && ` · ${PM_TYPE_LABELS[t.pm_type] || t.pm_type}`}
                  </p>
                </div>
